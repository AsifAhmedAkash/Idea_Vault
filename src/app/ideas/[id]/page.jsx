...
const IdeaDetailsPage = async ({ params }) => {
    ...
    if (!token) {
        redirect(`/login?callbackUrl=/ideas/${id}`);
    }
    ...
    return (
        <main className="w-full px-20 md:px-16 py-20 bg-[#fafaf3] dark:bg-zinc-950 text-[#1a1c18] dark:text-zinc-50">
            ...
            <div className="flex flex-wrap gap-2 mb-10">
                {tags?.map((tag) => (
                    <span key={tag} className="px-3 py-1 text-xs rounded-full border border-[#e3e3dc] dark:border-zinc-700 text-[#4c6700] dark:text-lime-400 font-medium">
                        #{tag}
                    </span>
                ))}
            </div>
